...
TEST_F(FileIOFixture, WriteOutput_OverwritesExistingLBA_ThroughReadCheck) {
  ...
  CommandBuffer cmdbuffer2{
      std::move(std::make_unique<WriteCommandOptimizer>())};
  SsdOperationHandler opHandler2(file_io, cmdbuffer2);
  opHandler2.WriteAndErase(pc);
...
class FileIOMockTest : public Test {
 protected:
  FileIOMock mock;
  CommandBuffer cmdBuf{std::move(std::make_unique<WriteCommandOptimizer>())};
