...
bool ReadCommand::IsValidParameter(int lba, std::string val) {
  if (!IsValidLba(lba)) {
    return false;
  }
  if (!IsValidValue(val)) {
...
bool ReadCommand::IsValidLba(int lba) {
  if (lba < MIN_VAL || lba > MAX_VAL) {
    return false;
  }
  return true;
}

bool ReadCommand::IsValidValue(std::string val) {
